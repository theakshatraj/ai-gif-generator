...
export default function AuthModal({ isOpen, onClose, mode = 'signin' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-full max-w-md mx-auto bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center animate-fadeIn">
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          <input
            ...
            required
            className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:outline-none text-base transition"
          />
          ...
          <button
            type="submit"
            className="w-full py-3 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition text-lg shadow focus:outline-none focus:ring-2 focus:ring-indigo-400"
            disabled={loading}
          >
        ...
        <div className="flex flex-col gap-2 mt-4 w-full">
          {view !== 'signin' && (
            <button className="text-indigo-600 hover:underline text-sm" onClick={() => { setView('signin'); setError(''); setSuccess(''); }}>Already have an account? Sign In</button>
          )}
          {view !== 'signup' && (
            <button className="text-indigo-600 hover:underline text-sm" onClick={() => { setView('signup'); setError(''); setSuccess(''); }}>Need an account? Sign Up</button>
          )}
          {view !== 'forgot' && (
            <button className="text-indigo-600 hover:underline text-sm" onClick={() => { setView('forgot'); setError(''); setSuccess(''); }}>Forgot Password?</button>
          )}
        </div>
        <div className="mt-6 w-full flex flex-col items-center">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 font-semibold text-gray-700 shadow-sm text-base transition"
          >
