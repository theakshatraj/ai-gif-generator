class ApiService {
  ...
  // NEW: Get YouTube video info (duration, title)
  async getYouTubeVideoInfo(youtubeUrl) {
    try {
      const response = await fetch(`${this.baseURL}/video-info?youtubeUrl=${encodeURIComponent(youtubeUrl)}`)
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        throw new Error(errorData.message || errorData.error || "Failed to get YouTube video info")
      }
      const data = await response.json()
      console.log("✅ YouTube video info fetched:", data)
      return data
    } catch (error) {
      console.error("❌ API Error (getYouTubeVideoInfo):", error)
      throw error
    }
  }
