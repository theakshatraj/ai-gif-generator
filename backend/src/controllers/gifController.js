...
export const generateGifs = async (req, res) => {
  ...
  try {
    ...
    // Normalize youtubeUrls (array of URLs)
    let urlList = [];
    if (youtubeUrls) {
    ...
    // If file upload, process as before
    if (uploadedFile) {
      ...
      for (const url of urlList) {
        try {
          // --- Begin single video logic (copy from old youtubeUrl block) ---
          let videoPath, videoInfo, transcript, isVideoDownloadSuccessful = false;
          const youtubeData = await videoService.getYouTubeData(url);
          ...
          if (videoPath) tempFiles.push(videoPath);

          // Handle segmentation (applies to all videos if present)
          if (req.body.isSegmented === "true") {
          ...
          try {
            ...
            throw new Error(`AI analysis failed: ${aiAnalysisError.message}`);
          }

          if (!moments || moments.length === 0) {
            throw new Error("No suitable moments found for GIF creation. Try a different prompt or video.");
          }

          // Generate GIFs
          const gifs = [];
          let generatedTextGifs = false;
          ...
          results.push({
            ...
            videoInfo,
            videoContentUsed: isVideoDownloadSuccessful,
            isSegmented: videoInfo.isSegmented || false,
            generatedTextGifs,
          });
        } catch (err) {
    ...
    // If only one YouTube URL, keep old logic for backward compatibility
    if (urlList.length === 1) {
      const youtubeUrl = urlList[0];
