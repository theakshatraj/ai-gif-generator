...
class GifService {
  ...
  async createGif(videoPath, moment, videoInfo) {
    ...
    // Always scale to 640px width for memory efficiency
    const scaleFilter = 'scale=640:-1:flags=lanczos';

    let command = `${this.ffmpegPath} -ss ${moment.startTime} -i "${videoPath}" -t ${duration} -vf "fps=10,${scaleFilter},split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 "${outputPath}"`
    ...
    if (moment.caption && fs.existsSync(this.fontPath)) {
      ...
      const fontSize = 24
      command = `${this.ffmpegPath} -ss ${moment.startTime} -i "${videoPath}" -t ${duration} -vf "fps=10,${scaleFilter},drawtext=text='${cleanCaption}':fontfile='${this.fontPath.replace(/\\/g, "/")}':fontsize=${fontSize}:fontcolor=white:x=(w-text_w)/2:y=h-line_h-10:box=1:boxcolor=black@0.7:boxborderw=2,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 "${outputPath}"`
      ...
      const fontSize = 24
      command = `${this.ffmpegPath} -ss ${moment.startTime} -i "${videoPath}" -t ${duration} -vf "fps=10,${scaleFilter},drawtext=text='${cleanCaption}':fontsize=${fontSize}:fontcolor=white:x=(w-text_w)/2:y=h-line_h-10:box=1:boxcolor=black@0.7:boxborderw=2,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 "${outputPath}"`
    }
  ...
  async createTextGif(moment, videoInfo) {
    ...
    let command = `${this.ffmpegPath} -f lavfi -i color=${backgroundColor}:s=640x360:d=${duration} -vf "drawtext=text='${cleanCaption}':fontfile='${this.fontPath.replace(/\\/g, "/")}':fontsize=${fontSize}:fontcolor=${textColor}:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.7:boxborderw=2,fps=10,${scaleFilter},split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -y "${outputPath}"`

    if (!fs.existsSync(this.fontPath)) {
      console.warn(`⚠️ Font file not found at ${this.fontPath}. Text GIFs will be generated without custom font.`)
      command = `${this.ffmpegPath} -f lavfi -i color=${backgroundColor}:s=640x360:d=${duration} -vf "drawtext=text='${cleanCaption}':fontsize=${fontSize}:fontcolor=${textColor}:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.7:boxborderw=2,fps=10,${scaleFilter},split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -y "${outputPath}"`
    }
